...
class AddToScheduleForm(forms.Form):
  ...
  script = forms.FileField(required=False, label='Select a script for MobiBed',
          help_text='max. 42 megabytes')
  contrib = forms.FileField(required=False, label='Select contributed dex file for MobiBed',
          help_text='max. 42 megabytes')
  ...
  def Add(self, **unused_args):
    ...
    if not self.request.POST:
      ...
      if add_to_schedule_form.is_valid():
        ...
        elif task.type == 'mobibed':
          script = self.request.get('script')
          logging.info(type(script))
          if script:
            task.script = db.Text(script, encoding="utf_8")
            logging.info('save script')
          else:
            print 'no script error.'
            raise
          try:
            contrib = self.request.POST.get('contrib').file.read()
            task.contrib = db.Blob(contrib)
            task.contrib_url = str(long(time.time()*1000))
            logging.info('save contrib')
